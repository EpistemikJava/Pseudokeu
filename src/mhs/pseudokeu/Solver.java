...
class Solver {
    ...
    private int dataLeft(final int i) {
        return i - 1 < 0 ? nCols - 1 : i - 1;
    }

    private int dataRight(final int i) {
        return (i + 1) % nCols;
    }

    private int dataUp(final int i) {
        return i - 1 < 0 ? nRows - 1 : i - 1;
    }

    private int dataDown(final int i) {
        return (i + 1) % nRows;
    }
    ...
    private void loadGame(final SavedGame game) {
        ...
        for( int $row = 0; $row < $len; $row++ )
            for( int $col = 0; $col < $len; $col++ ) {
                ...
                if( $val != SqrTypes.BLANK_VAL ) includeNode($val - 1, $row, $col);
            }
    ...
    private void search(final int nodeCount) {
        ...
        for( $downNode = $nextNode.down; ($downNode != $nextNode) && !done; $downNode = $downNode.down ) {
            ...
            if( !done ) {
                ...
                logger.fine("Ok, " + ($val + 1) + " at r" + ($row + 1) + "c" + ($col + 1) + " (node " + (nodeCount + 1)
                                + ") didn't work...");
        ...
        logger.fine(" >> Finished search(" + nodeCount + "): next$node = " + $nextNode.cType + $nextNode.nID + "; down$Node = "
                        + $downNode.cType + $downNode.nID);
    ...
    private final int gridLen, nSqrs, nCols, nRows;

    private final int sqrOffset, rowOffset, colOffset, boxOffset;
