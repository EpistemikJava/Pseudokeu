/* ***************************************************************************************
 ...
 -----------------------------------------------
  ...
  independent git repository created Apr 4, 2015
...
public class Launcher extends JFrame {
    ...
    class GameListener implements ActionListener {
         ...
         * - EXCEPT those for Inner Classes, i.e. {@link GameSizeFrame}, {@link AddGameFrame}, {@link SelectGameFrame},
         * {@link ChooseDifficultyFrame} & {@link SolveDelayFrame}
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            } else // turn OFF Solve for these events
            {
                ...
                if( autoSolveActivated() ) toggleAutoSolve();
    ...
    class GameSizeFrame extends JFrame implements ActionListener {
        ...
        private void confirm() {
            ...
            if( pxSqrLength == newSqrSize ) return;
    ...
    class SelectGameFrame extends JFrame implements ActionListener, ChangeListener {
        ...
        static final String STR_TITLE = "Select a Game",
    ...
    class AddGameFrame extends JFrame implements ActionListener {
        ...
        static final String STR_ADDGAME = "Add Game",
                        STR_ADDANDCLOSE = "Add Game & Close",
                        HTML_MSG = "<html><br><h1><font color=blue><u>ADD a Game to the USER folder</u></font></h1>"
                                        + "<ul>"
        ...
        private int pxMsgHt = 300, pxCfmHt = 120, pxPrefWidth = 768, pxWidth, pxHeight;
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if( source == addBtn ) {
                ...
                if( confirm() ) launchAddGameFrame(false);
            } else if( source == cancelBtn ) {
    ...
    class ChooseDifficultyFrame extends JFrame implements ActionListener {
         ...
         */
        private void setComboBoxSelection(int sel) {
    ...
    public Launcher(boolean debugMode, String logLevel) {
        ...
        if( logControl != null ) logger = logControl.getLogger();

        if( logger == null ) exitInit("Constructor", "PROBLEM WITH LOGGER!");
    ...
    void toggleAutoSolve() {
        ...
        if( !(autoSolveActive || DEBUG || grid.hasEntries()) ) return;
    ...
    void setConflicts(boolean flip) {
        ...
        if( flip ) showConflicts = !showConflicts;
     ...
     */
    boolean autoSolveActivated() {
        return autoSolveActive;
    }
    ...
    void enableUndo(final boolean yes) {
        if( undoButton.isEnabled() != yes ) undoButton.setEnabled(yes);
    }
    ...
    void enableRedo(final boolean yes) {
        if( redoButton.isEnabled() != yes ) redoButton.setEnabled(yes);
    }
    ...
    void enableAutoSolve(final boolean yes) {
        if( solveButton.isEnabled() != yes ) solveButton.setEnabled(yes);
    }
    ...
    void updateTime(final String str) {
        timeLabel.setText(strTIME_TITLE + str);
    }
     ...
     */
    void updateInfoTitle(final String str) {
     ...
     * @param str - <CODE>String</CODE> with updated info
     */
    void updateInfoMesg(final String str) {
        infoMesg.setText(str);
    }
    ...
    private void initGameSize() {
        ...
        pxGameSize = new Dimension((pxSqrLength * INITIAL_GRID_LENGTH) + (X_BORDER * 2), (pxSqrLength * INITIAL_GRID_LENGTH)
                        + (PANEL_HEIGHT * NUM_MAIN_PANELS) + (Y_BORDER * 2));

        setSize(pxGameSize);

        logger.logInit("Initial Game width = " + pxGameSize.width + " & height = " + pxGameSize.height
                        + " & msg Frame width = " + pxMsgFrameWidth);
    ...
    /**
     * Enable {@link Grid}, {@link Loader} &amp; <code>Inner Class Frames</code>, then place the game on screen <br>
     * - called by {@link #main}
     * 
    ...
    private final void go() {
        ...
        if( !createGrid() ) exitInit("go()", "PROBLEM WITH GRID!");

        /* 2. MUST create the Loader AFTER the Grid has been created */
        if( !createLoader() ) exitInit("go()", "PROBLEM WITH LOADER!");
    ...
    private final void createLoadWorker() {
        ...
        loadWorker = new SwingWorker<Integer, Void>() {
            ...
            public Integer doInBackground() {
                return Integer.valueOf(loader.loadAllGames()
    ...
    private void undoLastEntry() {
        // turn OFF Solve if active
        if( autoSolveActive ) toggleAutoSolve();
     ...
     */
    private void revealSquare() {                 // stop clock if game is minimized
     ...
     */
    private void runClock() {                 // stop clock if game is minimized
        if( running && (getState() == NORMAL) ) {
    ...
    void setGridDebugKeyMap(InputMap gImap, ActionMap gAmap) {
        if( DEBUG ) {
            ...
            gAmap.put(strActive, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strSquare, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
                    grid.getActiveSqr().display(Level.SEVERE, LogControl.fine() ? Grid.FULL : Grid.BRIEF, strSquare);
                }
            });
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_S, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK), strSquares);
            gAmap.put(strSquares, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strRow, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK), strRows);
            gAmap.put(strRows, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strCol, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_C, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK), strCols);
            gAmap.put(strCols, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strZone, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK), strZones);
            gAmap.put(strZones, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strGroups, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
                    grid.displayActiveGroups(Level.SEVERE, LogControl.fine() ? Grid.FULL : Grid.BRIEF, strGroups);
                }
            });
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_G, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK),
                            strAllGroups);
            gAmap.put(strAllGroups, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strGrid, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strRunLockVals, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
                    grid.findLockedVals();
                }
            });
            ...
            gAmap.put(strShowSolution, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strPossValShow, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_P, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK),
                            strClearTempVals);
            gAmap.put(strClearTempVals, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strKeystrokes, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strMouseclicks, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gAmap.put(strMainLoggers, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
            ...
            gImap.put(KeyStroke.getKeyStroke(KeyEvent.VK_E, InputEvent.CTRL_DOWN_MASK | InputEvent.SHIFT_DOWN_MASK),
                            strRegLoggers);
            gAmap.put(strRegLoggers, new AbstractAction() {
                public void actionPerformed(ActionEvent ae) {
    ...
    // OpenSUSE = 6, Ubuntu = 7 (Feb 2012), git = 8 (Mar 2014)
                    strMAJOR_VERSION = "8", strMINOR_VERSION = "1.2", strVERSION_NUM = strMAJOR_VERSION + "."
                                    + strMINOR_VERSION, GAME_VERSION = PROJECT_NAME + "Version # " + strVERSION_NUM;

    /** game default */
    static final int MAX_NUM_LOADED_GAMES = 250, RANDOM_GAME = -1, THREAD_SLEEP_MSEC = 666, INITIAL_SOLVE_DELAY_MSEC = 2000,
                    DEBUG_SOLVE_DELAY_MSEC = 3000, USER_SOLVE_DELAY_MSEC = 4000;

    /** component dimension */
    static final int
    // in pixels
                    SQUARE_SIZE_SM = 48,
                    SQUARE_SIZE_MD = 60, SQUARE_SIZE_LG = 72,
                    ...
                    FONT_SIZE_MD = SQUARE_SIZE_MD / 2 + 4, FONT_SIZE_LG = SQUARE_SIZE_LG / 2 + 4;

    /** default background {@link Color} */
    static final Color COLOR_GAME_BKGRND = new Color(248, 232, 216); // beige

    /** message String */
    static final String strZERO_TIME = "00:00:00",
                    ...
                    htmlINSTRUCT_CONTENT = "<html><ul>"
                                    ...
                                    + "a Square is selected!</font></h3></li>" + "</ul></html>",
                    htmlABOUT_CONTENT = "<html><h2>Created by Mark Sattolo &copy; 2007</h2></html>" + "\n\n[ " + GAME_VERSION
                                    + " ]\n\n",

                    strEVENT = " event", strNewGame = "New " + strGAME, strSelectGame = "Select a " + strGAME + "...",
                    strAddGame = "Add a " + strGAME + "...", strExit = "eXit",
                    strLoadDelay = "Games have not finished loading -- Please wait a moment.", strSize = "Game Size...",
                    strDifficulty = "Difficulty Level...", strSolveDelay = "Delay for Solve...", strSettings = "Settings",
                    strInstruct = "How to Play...", strAbout = "About " + GAME_NAME,
                    strRevealWrong = "Reveal a WRONG value (if any)", strHelp = "Help", strExitCap = "EXIT", strUndo = "Undo",
                    strRedo = "Redo", strAccept = "Accept", strCancel = "Cancel", strConfirm = "Confirmation",
                    strConflicts = "Conflicts", strShowConflicts = "Show " + strConflicts, strHideConflicts = "Hide "
                                    + strConflicts, strHint = "Hint", strSolve = "Solve " + strGAME,
                    strStopSolve = "Stop Solving", strProblem = "PROBLEM", strSuccess = "SUCCESS", strLevel = "Level",

                    strALL_VALS_GOOD = "ALL values are good!", strNEED_USER_VALUE = "Need AT LEAST ONE user value...",
                    strSOLUTION_NOT_AVAIL = "Solution is not available yet.",
                    strNOT_ACTIVATE_GAME = "COULD NOT ACTIVATE GAME #", strNO_GAMES_LOADED = "NO GAMES LOADED!",
                    ...
                    strActive = "Show Active Items", strSquare = "Show Active Square",
                    strSquares = "Show Active & Group Squares", strGrid = "Show Grid Variables", strRow = "Show Current Row",
                    strRows = "Show ALL Rows", strCol = "Show Current Col", strCols = "Show ALL Cols",
                    strZone = "Show Current Zone", strZones = "Show ALL Zones", strGroups = "Show Active Groups",
                    strAllGroups = "Show ALL Groups", strKeystrokes = "Toggle Keystrokes",
                    strMouseclicks = "Toggle Mouseclicks", strMoreLogging = "Increase GAME Logging",
                    strLessLogging = "Decrease GAME Logging", strMainLoggers = "Info on Main Loggers",
                    strRegLoggers = "Show Registered Loggers", strPossValShow = "Set Possible Vals as Temp Vals",
                    strClearTempVals = "Clear ALL Temp Vals", strRunLockVals = "Run the Locked Values algorithm",
                    ...
                    strF7tooltip = "F7 - Undo the last value set in the Grid", strF8tooltip = "F8 - Redo the last UNDO action",
    ...
    private JMenuItem newGameMenuItem, selectGameMenuItem, addGameMenuItem, exitMenuItem;
    ...
    private JMenuItem solveDelayMenuItem, difficultyMenuItem;
    ...
    private JMenuItem instructMenuItem, aboutMenuItem, revealWrongMenuItem;
    ...
    private JLabel sqrsLabel, timeLabel;
    ...
    private JLabel infoTitle, infoMesg;
    ...
    private JButton newGameButton, conflictsButton, hintButton;
