...
/**
 * Used by {@link Square} <var>type</var> to define different types of Squares and to centralize various useful {@link Square}
 * properties
 * 
 * @author Mark Sattolo
 */
enum SqrTypes {
    ...
    static final int BLANK_VAL = OPEN.getKey(),
...
class Square {
    ...
    void newValue(final int val, final boolean tempMode) {
        ...
        if( ((val == SqrTypes.BLANK_VAL) && (value == SqrTypes.BLANK_VAL)) || ((val == value) && (temp == tempMode)) ) {
            logger.fine("DO NOTHING... current val = " + value + (temp ? "/T" : "") + " & new val = " + val
                            + (tempMode ? "/T" : "") + " // current type = " + type);
            return;
        }
        ...
        else if( isOpen() ) $newVal = SqrTypes.BLANK_VAL;
    ...
    void adjustGroupSqrCounts(final int oldVal, final int newVal) {
        ...
        if( oldVal != SqrTypes.BLANK_VAL ) --nGrpSqrsWithVal[oldVal];

        if( newVal != SqrTypes.BLANK_VAL ) ++nGrpSqrsWithVal[newVal];
    ...
    boolean canBeVal(int val) {
        if( ((1 << val) | possibleVals) == possibleVals ) return true;
    ...
    /**
     * Reset all mutable fields to default values - called by {@link #Square(Grid)} and {@link Grid#clear}
     */
    final void clear() {
    ...
    /**
     * This {@link Square} has a <var>value</var> the same as at least one other {@link Square} in one of its {@link Group}s
    ...
    int setPossValsAsTempVal() {
        if( (!isOpen()) || (nPossibleVals > 3) ) return SqrTypes.BLANK_VAL;
        ...
        if( $count > 0 ) newValue($tempVal, true);
    ...
    void display(final Level lev, final boolean brief, final String info) {
        ...
        logger.append(" Sqr " + strGridPosn() + "_z" + myZone.getPosn() + (active ? "/A" : "") + " = " + value
                        + (temp ? "/temp" : "") + (autoSolved ? "/SOLVED" : "") + " (" + (conflicting ? "/CONFLICT " : "")
                        + (wrong ? "/WRONG " : "") + type + "): nPV." + nPossibleVals + " SV." + solvedValue + " PV."
                        + possibleVals);
    ...
    /**
     * Do I have a 'conflicting' <var>value</var>, i.e. one duplicated in another {@link Square} in one of my {@link Group}s?
     */
    private boolean conflicting;

    /**
     * Do I have a 'wrong' <var>value</var> in a {@link Square}, i.e. NOT the proper value for the solution?
     */
