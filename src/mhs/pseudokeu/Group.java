...
public abstract class Group {
    ...
    void changedSqr(final int oldval, final int newval, final SqrTypes oldtype, final SqrTypes newtype) {
        ...
        if( oldtype != newtype ) adjustTypeCounts(oldtype, newtype);

        if( (oldtype == SqrTypes.GUESS) && (--nSqrsWithVal[oldval] == 0) ) nVals--;

        if( (newtype == SqrTypes.GUESS) && (++nSqrsWithVal[newval] == 1) ) nVals++;
    ...
    Group getCommonGroup(final int sqr1, final int sqr2, final int sqr3) {
        ...
        if( g == mySqrs[sqr2].getZone() ) {
            $result = true;
            if( (sqr3 > 0) && (g != mySqrs[sqr3].getZone()) ) $result = false;
        }
    ...
    boolean findSqrPairs() {
        ...
        if( nOpen > 2 ) {
            ...
            for( int i = 0; i < gridLength; i++ ) {
                if( mySqrs[i].numPossibleVals() == 2 ) for( int j = i + 1; j < gridLength; j++ )
    ...
    boolean findGrpPairs() {
        ...
        if( nOpen > 2 ) {
            ...
            for( int v = 1; v <= gridLength; v++ ) {
                if( nSqrsCanBeVal[v] == 2 ) for( int w = v + 1; w <= gridLength; w++ )
    ...
    boolean findSqrTriples() {
        ...
        if( nOpen > 3 ) {
            ...
            loop: for( int i = 0, j = 0, k = 0; i < gridLength; i++ )
                        ...
                        if( (mySqrs[j].numPossibleVals() == 2) || (mySqrs[j].numPossibleVals() == 3) ) {
                            ...
                            for( k = j + 1; k < gridLength; k++ )
                                if( (mySqrs[k].numPossibleVals() == 2) || (mySqrs[k].numPossibleVals() == 3) ) {
                                    ...
                                    if( $res2 == $res1 ) {
                                        logger.appendln("\n\t nOpen == " + nOpen + " / i == " + i + " ; j == " + j + " ; k == "
                                                        + k + " ; res1 == " + Helper.displaySetBits($res1, gridLength, "")
    ...
    boolean findGrpTriples() {
        ...
        if( nOpen > 3 ) {
            ...
            loop: for( int v = 1, w = 1, x = 1; v <= gridLength; v++ )
                if( (nSqrsCanBeVal[v] == 2) || (nSqrsCanBeVal[v] == 3) ) for( w = v + 1; w <= gridLength; w++ )
                        ...
                        if( (nSqrsCanBeVal[x] == 2) || (nSqrsCanBeVal[x] == 3) ) {
                            ...
                            if( $res2 == $res1 ) {
                                logger.appendln("\n\t nOpen == " + nOpen + " / v == " + v + " ; w == " + w + " ; x == " + x
                                                + " ; res1 == " + Helper.displaySetBits($res1, gridLength, "") + " ; res2 == "
                ...
                if( (mySqrs[i].numPossibleVals() >= 2) && (mySqrs[i].numPossibleVals() <= 4) )
                    for( int j = i + 1; j < gridLength; j++ )
                        if( (mySqrs[j].numPossibleVals() >= 2) && (mySqrs[j].numPossibleVals() <= 4) ) {
                            ...
                            if( Helper.numSetBits($res1, gridLength) > 4 ) continue;
                            ...
                            for( int k = j + 1; k < gridLength; k++ )
                                if( (mySqrs[k].numPossibleVals() >= 2) && (mySqrs[k].numPossibleVals() <= 4) ) {
                                    ...
                                    if( Helper.numSetBits($res2, gridLength) != 4 ) continue;
    ...
    boolean findGrpQuads() {
        ...
        if( nOpen > 4 ) {
            ...
            loop: for( int v = 1; v <= gridLength; v++ )
                if( (nSqrsCanBeVal[v] >= 2) && (nSqrsCanBeVal[v] <= 4) ) for( int w = v + 1; w <= gridLength; w++ )
    ...
    String myPosn() {
        return(this.getClass().getSimpleName() + "[" + posnIndex + "]");
    }
    ...
    /**
     * Keep track (using bits for the indices of {@link #mySqrs}) of which {@link Square}s can be each of my values
     */
    int[] sqrsCanBeVal;
...
class Zone extends Group {
    ...
    static final Color[] COLORS = { Color.PINK, Color.yellow.darker(), new Color(180, 80, 254), // violet
                    Color.ORANGE, Color.GRAY.brighter(), Color.CYAN, Color.GREEN.brighter(), Color.MAGENTA,// new Color( 254,
                                                                                                           // 99, 200 ), //
                                                                                                           // lighter than
                                                                                                           // MAGENTA ,
    ...
    Group getCommonGroup(final int sqr1, final int sqr2, final int sqr3) {
        ...
        if( g == mySqrs[sqr2].getRow() ) {
            $result = true;
            if( (sqr3 > 0) && (g != mySqrs[sqr3].getRow()) ) $result = false;
        }
        ...
        if( g == mySqrs[sqr2].getCol() ) {
            $result = true;
            if( (sqr3 > 0) && (g != mySqrs[sqr3].getCol()) ) $result = false;
        }
