...
public class Loader {
    ...
    class SavedGame {
        ...
        protected SavedGame(final int len, final String str, final int diff) {
            this(len, str);
            difficulty = diff;
            logger.info("name = " + this.getName() + "; length = " + this.getLength() + "; difficulty = "
                            + this.getDifficulty());
        }

        String myname() {
            return SavedGame.class.getSimpleName();
        }

        String fullname() {
            return SavedGame.class.getName();
        }
        ...
        protected int setValue(final int row, final int col, final int val) {
            int $val = val, $oldVal = values[row][col];
        ...
        void display(final Level lev) {
            ...
            if( !LogControl.atLevel(level) ) return;

            logger.appendln("GAME '" + name + "' : length = " + length + " & difficulty = "
                            + Loader.STR_DIFF_FOLDERS[difficulty] + "\n");
    ...
    boolean addGame(final Grid grid) {
        ...
        if( grid.getNumValues() <= 0 ) return boolFAIL;
    ...
    String getFileName(final int difficulty) {
        ...
        if( difficulty == USER ) return userPrefix;
    ...
    int getGameIndex(final int difficulty, final String name) {
        ...
        for( int i = 0; i < nLoadedGames[difficulty]; i++ )
            if( games[difficulty][i].getName().contentEquals(name) ) return i;
    ...
    int getDifficultyIndex(final String difficulty) {
        ...
        for( int i = 0; i < maxNumGames; i++ )
            if( STR_DIFF_FOLDERS[i].contentEquals(difficulty) ) return i;
    ...
    private boolean scanFile(final File file) {
        ...
        if( loadscanner != null ) $result = true;
    ...
    private boolean scanJar(final String name) {
        ...
        if( loadscanner != null ) $success = true;
...
class Helper {
    ...
    static int numSetBits(final int val, final int lim) {
        ...
        int $limit = lim;
        if( $limit > Integer.SIZE ) $limit = Integer.SIZE;
    ...
    static int getBitPosn(final int val, final int place) {
        if( (place < 1) || (place > Integer.SIZE) ) return 0;
